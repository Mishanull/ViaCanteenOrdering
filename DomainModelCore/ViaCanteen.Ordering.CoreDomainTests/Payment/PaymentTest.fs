...
let ``Test CreditCard Payment Execution``() =
    let creditCard = new CreditCard("1234-5678-9101-1121", DateTime(2024, 12, 31), 100.0)
    Assert.DoesNotThrow(fun () -> creditCard.ExecutePayment())

[<Test>]
let ``Test MobilePay Payment Execution``() =
    let mobilePay = new MobilePay("0800123456", 50.0)
    Assert.DoesNotThrow(fun () -> mobilePay.ExecutePayment())
